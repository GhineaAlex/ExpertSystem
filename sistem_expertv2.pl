...
executa([_|_]) :-
	...
	scopuri_princ:-
	scop(Atr), (setof(st(FC,Atr,Val), Istorie^(determina(Atr), fapt(av(Atr,Val), FC, Istorie), FC>49),L) ->afis_lista_sol(L); write('Nu s-a gasit moneda')),nl.
...
realizare_scop(not av(Atr,_),FC,_) :-
fapt(av(Atr,nu_stiu),FC,_), !.
...
transformare2(not av(A,da), ['fals',A]) :- !.

transformare2(av(A,nu),['fals',A]) :- !.
...
citeste_opt(X,Optiuni,Istorie) :-
	append(Optiuni,[nu_stiu,nu_conteaza],Optiuni2),
	append(['('],Optiuni2,Opt1),
	append(Opt1,[')'],Opt),
	scrie_lista(Opt),
	de_la_utiliz(X,Istorie,Optiuni2).
...
trad(interogabil(Atr,M,P)) --> [intrebare, '[', text_intrebare, '=', '[', P, ']', atribut, '=', '[', Atr, ']', optiuni, '=', '['], lista_de_optiuni(M), [ ']'].
...
daca(Daca) --> [daca,sunt,adevarate,conditiile,':'], lista_premise(Daca).
lista_premise([Daca]) --> propoz_premisa(Daca), [rezulta].
lista_premise([Prima|Celalalte]) --> propoz_premisa(Prima), lista_premise(Celalalte).

propoz_premisa(not av(Atr, da)) --> [fals, '[', Atr,']'].
propoz_premisa(av(Atr, Val)) --> ['[', Atr, ']', '=', '[', Val, ']'].
propoz_premisa(av(Atr, da)) --> ['[', Atr, ']'].

atunci(Atunci, FC) --> propoz_concluzie(Atunci), [cu, fact_cert, '=', '[', FC, ']'].
...
caracter_cuvant(C):-member(C,[34,44,64,91,93,58,35,60,45,47,61,91,92,93,123,124,125]).
